...
cat <<EOF > output-${APPLICANT}-${CLUSTER_NAME}/role.yaml
kind: Role
...
kubectl apply -f output-${APPLICANT}-${CLUSTER_NAME}/role.yaml --kubeconfig $ADMIN_KUBE_CONFIG

cat <<EOF > output-${APPLICANT}-${CLUSTER_NAME}/rolebinding.yaml
kind: RoleBinding
apiVersion: rbac.authorization.k8s.io/v1beta1
metadata:
  name: ${APPLICANT}-${TEAM}-admin-binding
  namespace: $APPLICANT_NS
subjects:
  - kind: ServiceAccount
    name: ${APPLICANT}-${TEAM}
...
kubectl apply -f output-${APPLICANT}-${CLUSTER_NAME}/rolebinding.yaml --kubeconfig $ADMIN_KUBE_CONFIG
